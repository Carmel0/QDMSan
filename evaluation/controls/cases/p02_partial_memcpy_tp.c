 ...
 * Use:      the 16-byte prefix plus dst[16] (first uninitialized byte) are
 *           used as table indices (memory-address consumption).
 */
#include <stdlib.h>
#include <string.h>
...
int main(void) {
  ...
  unsigned acc = 0;
  for (int i = 0; i < K + 1; i++) acc += lut[dst[i]];
  sink = acc;
